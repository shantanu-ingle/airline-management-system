            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'ec2-ssh-key', keyFileVariable: 'SSH_KEY', usernameVariable: 'SSH_USER')]) {
                    bat """
                        ...
                        C:\\Windows\\System32\\OpenSSH\\scp.exe -o ConnectTimeout=30 -o StrictHostKeyChecking=no -i "%SSH_KEY%" target\\airline-management-system-1.0.jar %SSH_USER%@54.159.204.82:/home/%SSH_USER%/
                        ...
                        C:\\Windows\\System32\\OpenSSH\\ssh.exe -o ConnectTimeout=30 -o StrictHostKeyChecking=no -i "%SSH_KEY%" %SSH_USER%@54.159.204.82 "
                            ...
                            sleep 5;
                            nohup java -jar /home/%SSH_USER%/airline-management-system-1.0.jar --server.address=0.0.0.0 > /home/%SSH_USER%/app.log 2>&1 &;
                            sleep 20;
                ...
                script {
                    retry(5) {
                        timeout(time: 2, unit: 'MINUTES') {
                            bat """
                            curl -v --retry 5 --retry-delay 10 --max-time 30 http://54.159.204.82:8081/flights
                            """
                        }
                    }
                }
