                ...
                script {
                    retry(5) {
                        timeout(time: 3, unit: 'MINUTES') {
                            ...
                            echo Creating test data for flights...
                            curl -v --retry 5 --retry-delay 10 --max-time 30 -X POST -H "Content-Type: application/json" -d "{\\"flightNumber\\":\\"AA123\\",\\"destination\\":\\"New York\\",\\"departureTime\\":\\"2025-05-16T10:00:00\\",\\"arrivalTime\\":\\"2025-05-16T12:00:00\\"}" http://13.220.119.113:8081/api/flights

                            echo Creating test data for tickets...
                            curl -v --retry 5 --retry-delay 10 --max-time 30 -X POST -H "Content-Type: application/json" -d "{\\"flightId\\":1,\\"passengerName\\":\\"John Doe\\",\\"seatNumber\\":\\"12A\\"}" http://13.220.119.113:8081/api/tickets

                            echo Validating GET /api/flights?sort=asc...
                            curl -v --retry 5 --retry-delay 10 --max-time 30 "http://13.220.119.113:8081/api/flights?sort=asc"

                            echo Validating GET /api/flights/{id}...
                            curl -v --retry 5 --retry-delay 10 --max-time 30 http://13.220.119.113:8081/api/flights/1

                            echo Validating GET /api/tickets/{id}...
                            curl -v --retry 5 --retry-delay 10 --max-time 30 http://13.220.119.113:8081/api/tickets/1

                            echo Validating DELETE /api/tickets/{id}...
                            curl -v --retry 5 --retry-delay 10 --max-time 30 -X DELETE http://13.220.119.113:8081/api/tickets/1
                            """
                        }
                    }
                }
